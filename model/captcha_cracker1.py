...
def Train(train_fn, image_input, target_chars, count_imagestrained_sofar, eval_matrix,
    ...
    for image_input_batch, target_chars_batch in IterateMinibatches(
        ...
        print("trained  {0}  images".format(total_images_trained),
              "  Batch training took {:.3f}s".format(batch_training_time),
              "  training loss: {:.6f}".format(batch_training_loss))
...
def Test(test_fn, image_input, target_chars, total_images_trained, train_flag, eval_matrix,
    ...
    if train_flag == 0:
        print("Testing on train data took {:.3f}s".format(time.time() - start_time))
        eval_matrix.number_of_images_training_acc.append((total_images_trained, char_acc, seq_acc))
    elif train_flag == 1:
        print("Testing on valid data took {:.3f}s".format(time.time() - start_time))
        eval_matrix.number_of_images_testing_acc.append((total_images_trained, char_acc, seq_acc))
    else:
        print("Testing on test data took {:.3f}s".format(time.time() - start_time))
        eval_matrix.number_of_images_testing_acc.append((total_images_trained, char_acc, seq_acc))
...
def _SaveModelAndRemoveOldOnes(captcha_model, prefix, n=50):
...
def Run( num_epochs=100, multi_chars=True, num_softmaxes=None):
    # training_data_dir = args.TrainingDirc
    # val_data_file = args.ValidateDirc
    val_data_file = "/Users/rookie/antiSpider/imgsrc/ydzhejiang/t2/training_images_1.npy.npz"
    ...
    learning_rate = 0.0002  # args.learningrate
    # lstm_grad_clipping = args.lstm_grad_clipping
    # if lstm_grad_clipping is None:
    ...
    im = Image.fromarray(val_image_input)
    im.save("/Users/rookie/antiSpider/imgsrc/ydzhejiang/t2/2.png")
    ...
    #      test_target_chars, total_images_trained, 2, eval_matrix, batch_size=TEST_BATCH_SIZE, multi_chars=multi_chars,
    #      use_mask_input=args.use_mask_input)
...
class CaptchaCracker(object):
    def __init__(self, model_params_file_prefix, includeCapital,
                 ...
                 num_rnn_steps=6, use_mask_input=False, lstm_layer_units=256, cnn_dense_layer_sizes=[256],
