...
parser.add_argument('--output', required=True, help='path to save pics')
parser.add_argument('--count', required=False, help='pic count')
...
if __name__ == '__main__':
    ...
    i = 0
    count = 10
    if opt.count is not None and opt.count != '':
        count = int(opt.count)

    while i < count:
        i += 1
        text, image = gen_captcha_text_and_image()
        file_name = opt.output + "/" + str(int(time.time() * 100000)) + "_" + text + ".png"
